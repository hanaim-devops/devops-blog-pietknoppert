...
<img src="https://miro.medium.com/v2/resize:fit:1100/format:webp/0*pxjok2ACCc7PJOCM.jpg" width="600">

# Wat is Thanos en hoe redt het je?
Thanos is een extensie voor Prometheus die schaalbaarheid, beschikbaarheid en langdurige opslag van metrics verbetert, en afhankelijk is van Prometheus voor zijn basisfunctionaliteit.

Weet je al wat Prometheus (Prometheus, z.d.) is? Thanos (Thanos Metrics, z.d) is namelijk afhankelijk op Prometheus. 
Het is handig om te weten wat Prometheus doet, ik raad je aan om een korte introductie over Prometheus te lezen van: Tigera, 2024 of de blog van mijn collega Mitchel te lezen!

## Thanos componenten
Laten we eerst kijken uit welke onderdelen Thanos bevat en welke interactie er is met Prometheus.

### Sidecar
Eén van de belangrijkste component is de ‘sidecar’ (Thanos Metrics, z.d.-d).
De sidecar fungeert als interface tussen Prometheus en Thanos, en uploadt block data naar een long-term storage (What Is Block Storage?, Oracle. z.d.).
Zoals een object store (data wordt opgeslagen als object: What Is Object Storage? Use Cases & Benefits | Google Cloud, z.d.)
bijvoorbeeld Amazon S3 (Amazon S3 - Cloud Object Storage - AWS, z.d.).
...
### Querier
Als je één of meerdere Sidecars hebt opgezet bij verschillende kun je doormiddel van de Thanos Querier queries over alle instanties heen uitvoeren. 
...
<img src="https://thanos.io/tip/img/querier.svg" width="500">
Bron: Thanos Metrics, z.d.-f

### Downsampling
Thanos ‘downsampling mechanisme’ wordt uitgevoerd door de Compactor component (Thanos Metrics, z.d.-d), die verantwoordelijk is voor het optimaliseren en comprimeren van de opgeslagen metrics-data. 
Thanos gebruikt hiervoor drie verschillende graten van dataopslag:

1.	Raw data (5s resolutie): De volledig metrics zoals ze door Prometheus zijn verzameld.
...
3.	1h downsampled data: Voor de langetermijnarchivering waarbij data wordt geaggregeerd over intervallen van een uur. Deze resolutie is voor het bekijken van historische trends over een lange periode.
...
Eén component die ik in deze blog niet heb behandeld is de Ruler. Dit is component zorgt voornamelijk voor het evalueren van Prometheus alerting en recording rules (Prometheus, z.d.-c).

## Verbinding met Prometheus
Prometheus heeft onderanderen beperkingen op het gebied van schaalbaarheid en long-term dataopslag: 'Prometheus's local storage is not intended to be durable long-term storage; external solutions offer extended retention and data durability.' (Prometheus, z.d.-b).

### Welke problemen lost Thanos op?
Thanos richt zich op onder andere deze onderwerpen:
1. **Lange-termijn opslag en schaalbaarheid**: Thanos gebruikt object storage waardoor metrics jarenlang kunnen worden bewaard zonder dat dure lokale opslag nodig is (Thanos metrics, z.d.-c). 
...
5. **Multi-cluster monitorin**g: Thanos biedt naadloze ondersteuning voor multi-clusteromgevingen, waarbij metrics van verschillende locaties worden samengebracht in één overzicht. Dit maakt het geschikt voor gedistribueerde cloud-native omgevingen (Thanos metrics, z.d.).
...
Ik ga nu een stukje hand-ons behandelen waar ik een voorbeeld applicatie met Thanos behandel. Om mee te doen moet je al Docker (Docker: Accelerated Container Application Development, 2024) geïnstalleerd hebben. 

Om het in dit voorbeeld behapbaar te laten heb ik een redelijk eenvoudige Docker compose gemaakt met enkele extra configuraties. Dit code kan je vinden in de GitHub repo bij: `src/project`
...
Dit zijn allemaal configuratie bestanden. Bijvoorbeeld in `prometheus.yml` staat welke containers hij moet gaan scrapen:
```
  ...
  - job_name: 'test-api'
    static_configs:
...
En in een Grafana configuratie (Grafana is eigenlijk optioneel maar wel leuk om de resultaten te zien) staat bijvoorbeeld een dashboard met visuele weergeven van de test-api:
...
En uiteraard staat in de `docker-compose.yml` alle verschillende services met benodigde informatie zoals naam, port, image en eventuele start commando. Hier een klein stukje van die compose:
```
services:
  ..   
  thanos-sidecar:
...
Nog even handig om te weten: de test-api is een simpel Python programma dat met een random functie, een succes (200) terug stuurd of een: fail (500). Deze staat publiek beschikbaar als een image, maar kan je ook vinden in het project. 
...
- **Prometheus** (`http://localhost:9090`)
  - Hier kun je de status van het systeem controleren en PromQL-queries (Prometheus (z.d.-d) uitvoeren om metrieken te analyseren. Prometheus verzamelt en bewaart metrieken van verschillende bronnen, waaronder de `test-api`, en stelt je in staat om gedetailleerde statistieken en grafieken te maken.
...
- **Grafana** (`http://localhost:3000`). Login met de username: admin en wachtwoord: admin
  - Grafana wordt gebruikt voor het visualiseren van de metrieken in dynamische en interactieve dashboards. Het biedt integraties met meerdere databronnen, zoals Thanos, en stelt je in staat om aangepaste dashboards te maken met verschillende soorten grafieken en monitoringwidgets. Grafana behoort niet tot de scope van dit onderzoek maar kan toch leuk zijn om met een visuele weergave Thanos in zijn werking te zien.
...
Neem een kijkje door de verschillende webapplicaties heen om te zien hoe het werkt. Bij Thanos kun je bijv. overzichtelijk zien hoe de sidecar is verbonden aan de test-api, ga naar: http://localhost:19090/stores. 

Gebruik de commando: `sum by (status) (request_count_total)` in Thanos (en- of Prometheus) om de resultaten van de test-api te zien. Dit ziet er bijvoorbeeld zo uit:
![img.png](plaatjes/result-thanos-query.png)

Controleer of bijvoorbeeld Prometheus dezelfde informatie laat zien. Als dat laatste ook is gelukt, neem dan een kijkje bij het dashboard van Grafana. Dit ziet er zo uit:
![img_1.png](plaatjes/result-grafana-test-api-dashboard.png)

_Deze configuratie is een eenvoudige setup met weinig schaalbaarheid, je ziet mogelijk niet meteen de volledige voordelen van Thanos._

# Voor en nadelen 
Voordelen van Thanos zijn: 

1. **Schaalbaarheid en Lange-termijn opslag**: Thanos maakt het eenvoudig om metrics op te slaan in schaalbare object storage zoals S3, waardoor gegevens veilig bewaard kunnen worden voor lange termijn analyses. Dit maakt het geschikt voor multi-cluster monitoring en grote datasets (Valialkin, 2021).
...
4. **Flexibiliteit en Integratie**: Thanos integreert met bestaande Prometheus-omgevingen zonder de noodzaak om veel wijzigingen aan te brengen. Ook kan je Stapsgewijs invoeren en hoeft niet de hele monitoring architectuur op de kop. Veel bedrijven introduceren Prometheus al vrij snel in hun project, terwijl Thanos later altijd nog toegevoegd kan worden. (Quiroz-Vázquez, 2024).
...
ontworpen voor lange-termijn opslag en multi-cluster monitoring. 
Het biedt een global query view en maakt het mogelijk om data uit meerdere bronnen te combineren.

Een anderen monitoringtool is: VictoriaMetrics. Deze onderscheidt zich door zijn eenvoud en hoge prestaties bij: data-inname, verwerking en grootschalige datasets. 
Het biedt een directe integratie met Prometheus en vereist geen complexe componenten zoals sidecars. 
...
OpenTelemetry biedt de breedste flexibiliteit door ondersteuning voor metrics, traces en logs. 
...
# Conclusie
Thanos is een krachtige extensie voor Prometheus die schaalbaarheid, hoge beschikbaarheid en langdurige opslag van metrics mogelijk maakt. 
...
en geavanceerde analysemogelijkheden te bieden. Hoewel de setup complex kan zijn en kosten voor object storage hoger kunnen uitvallen, 
