...
### Sidecar
Eén van de belangrijkste componenten is de ‘sidecar’ (Thanos Metrics, z.d.-d).
...
Zoals een object store (data wordt opgeslagen als object, zie Google Cloud, z.d.)
...
### Querier
Als je één of meerdere Sidecars hebt opgezet bij verschillende Prometheus-instanties, kun je door middel van de Thanos Querier queries over alle instanties heen uitvoeren. 
...
Eén component die ik in deze blog niet heb behandeld is de Ruler. Dit component zorgt voornamelijk voor het evalueren van Prometheus alerting en recording rules (Prometheus, z.d.-c).
...
Ik ga nu een stukje hands-on behandelen waar ik een voorbeeld applicatie met Thanos behandel. Om mee te doen moet je al Docker (Docker: Accelerated Container Application Development, 2024) geïnstalleerd hebben. 
...
Deze omgeving monitort systeem- en applicatieprestaties met Prometheus voor het verzamelen van metrieken, uitgebreid door Thanos voor long-term storage en multi-clusteranalyse. 
...
_Deze configuratie is een eenvoudige setup met beperkte schaalbaarheid; je ziet mogelijk niet meteen de volledige voordelen van Thanos._

# Voor- en nadelen 
Voordelen van Thanos zijn: 

1. **Schaalbaarheid en long-term storage**: Thanos maakt het eenvoudig om metrics op te slaan in schaalbare object storage zoals S3, waardoor gegevens veilig bewaard kunnen worden voor lange termijn analyses. Dit maakt het geschikt voor multi-cluster monitoring en grote datasets (Valialkin, 2021).
...
4. **Flexibiliteit en Integratie**: Thanos integreert met bestaande Prometheus-omgevingen zonder de noodzaak om veel wijzigingen aan te brengen. Ook kun je het stapsgewijs invoeren en hoeft niet de hele monitoringarchitectuur op de kop. Veel bedrijven introduceren Prometheus al vrij snel in hun project, terwijl Thanos later altijd nog toegevoegd kan worden. (Quiroz-Vázquez, 2024).
...
Daarnaast faciliteert Thanos integratie met CI/CD-pipelines, waardoor metrics automatisch kunnen worden geanalyseerd tijdens deployments. 
Het verbetert de feedbackloop tussen development en operations, bevordert continue verbetering en versterkt een DevOps-cultuur binnen de organisatie.
